...
class MinigridForwardDynamicsNet(nn.Module):
    ...
    def forward(self, obs, action):
        ...
        one_hot = torch.zeros(len(action),self.num_actions) #,device='cuda:5'
        for i in range(len(action)):

            #print('hey',action[i])
            action_tensor=torch.tensor([action[i].item()], dtype=torch.int32).long() #, device='cuda:5'
            #print('action tensor',action_tensor)
            one_hot[i] = torch.nn.functional.one_hot(action_tensor, num_classes=self.num_actions)
